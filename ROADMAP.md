...
*Linked processes that have their own diagram or child processes are rendered as collapsible `bpmn:SubProcess` shapes within the parent diagram. When collapsed, the shape is a compact task-sized box with a `+` marker at the bottom centre (standard bpmn-js SubProcess marker). When expanded, the shape grows inline on the same canvas and renders the full referenced process flow inside a large rounded rectangle — the parent diagram remains visible and the two flows share one continuous canvas. See `diagram.svg` for the reference visual: the top flow shows an expanded SubProcess containing `start → Task → end`; the bottom flow shows the same element collapsed.*

*Also covers automatic element-type switching driven by whether the linked process has depth.*

#### USER STORY 'Expand linked sub-process inline on the same canvas'
**AS A** process modeller\
**IF** I am viewing a BPMN diagram that contains a `bpmn:SubProcess` element linked to another process which has its own BPMN diagram\
**I WANT** to click the `+` marker on that collapsed sub-process shape and see the referenced process flow rendered inline inside an expanded rounded rectangle on the same canvas — exactly as bpmn-js renders a native expanded `bpmn:SubProcess` — without navigating away or opening a popup\
**SO THAT** I can read the complete parent-and-child flow in one continuous view

**Implementation note:** fetch the linked process's saved BPMN XML, embed its process elements and diagram plane as a nested SubProcess body within the parent definitions, set `isExpanded=true` on the shape, and re-import the merged XML so bpmn-js renders the expansion natively.

#### USER STORY 'Collapse an expanded inline sub-process'
**AS A** process modeller\
**IF** a sub-process is currently expanded inline showing its child flow\
**I WANT** to click the `−` marker (or the sub-process header) and have it collapse back to the compact task-sized box with the `+` marker\
**SO THAT** I can reduce visual noise and return to the high-level parent view without reloading the diagram

**Implementation note:** on collapse, strip the embedded subprocess body from the merged XML, set `isExpanded=false`, and re-import; the original `calledElement` reference is preserved so the link is not lost.

#### USER STORY 'Expand and collapse multiple sub-processes independently'
**AS A** process modeller\
**IF** a BPMN diagram contains more than one sub-process element each linked to a different process\
**I WANT** to expand and collapse each one independently — any combination of expanded and collapsed is valid simultaneously\
**SO THAT** I can compare selected parts of the process hierarchy side by side on a single canvas
