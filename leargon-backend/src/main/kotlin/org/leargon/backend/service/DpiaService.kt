...
open class DpiaService(
    ...
    @Transactional
    open fun triggerForProcess(processKey: String, currentUser: User): DpiaResponse {
    ...
    @Transactional
    open fun triggerForEntity(entityKey: String, currentUser: User): DpiaResponse {
    ...
    fun getDpiaForProcess(processKey: String): DpiaResponse {
        val process = processRepository.findByKey(processKey)
            .orElseThrow { ResourceNotFoundException("Process not found: $processKey") }
        val dpia = process.id?.let { dpiaRepository.findByProcessId(it) }
    ...
    fun getDpiaForEntity(entityKey: String): DpiaResponse {
        val entity = businessEntityRepository.findByKey(entityKey)
            .orElseThrow { ResourceNotFoundException("Entity not found: $entityKey") }
        val dpia = entity.id?.let { dpiaRepository.findByEntityId(it) }
    ...
    @Transactional
    open fun updateRiskDescription(dpiaKey: String, riskDescription: String?, currentUser: User): DpiaResponse {
    ...
    @Transactional
    open fun updateMeasures(dpiaKey: String, measures: String?, currentUser: User): DpiaResponse {
        val dpia = getDpiaByKey(dpiaKey)
        checkEditPermission(dpia, currentUser)
    ...
    @Transactional
    open fun updateResidualRisk(dpiaKey: String, residualRisk: String?, fdpicConsultationRequired: Boolean?, currentUser: User): DpiaResponse {
        val dpia = getDpiaByKey(dpiaKey)
        checkEditPermission(dpia, currentUser)
    ...
    @Transactional
    open fun complete(dpiaKey: String, currentUser: User): DpiaResponse {
        val dpia = getDpiaByKey(dpiaKey)
        checkEditPermission(dpia, currentUser)
    ...
    @Transactional
    open fun reopen(dpiaKey: String, currentUser: User): DpiaResponse {
        val dpia = getDpiaByKey(dpiaKey)
        checkEditPermission(dpia, currentUser)
    ...
    private fun getDpiaByKey(key: String): Dpia =
        dpiaRepository.findByKey(key)
            .orElseThrow { ResourceNotFoundException("DPIA not found: $key") }

    private fun checkEditPermission(dpia: Dpia, currentUser: User) {
