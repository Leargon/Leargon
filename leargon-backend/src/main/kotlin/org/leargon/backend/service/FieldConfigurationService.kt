...
open class FieldConfigurationService(
    ...
    @Transactional
    open fun getAll(): List<FieldConfigurationEntry> {
        return fieldConfigurationRepository.findAll().map { toEntry(it) }
    }
    ...
    data class FieldConfigResult(val mandatory: List<String>?, val missing: List<String>?)

    fun compute(entityType: String, isPresent: (String) -> Boolean): FieldConfigResult {
    ...
    private fun toEntry(config: FieldConfiguration): FieldConfigurationEntry {
        return FieldConfigurationEntry(config.entityType, config.fieldName)
    }
}
