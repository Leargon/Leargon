...
open class DpiaMapper {
    ...
    fun toDpiaListItemResponse(dpia: Dpia): DpiaListItemResponse {
        val triggeredBy = UserMapper.toUserSummary(dpia.triggeredBy)
            ?: throw IllegalStateException("DPIA ${dpia.key} has no triggeredBy user")
        val (resourceType, resourceKey, resourceName) = when {
            dpia.process != null -> Triple(
                DpiaListItemResponseLinkedResourceType.PROCESS,
                dpia.process!!.key,
                dpia.process!!.names.find { it.locale == "en" }?.text
                    ?: dpia.process!!.names.firstOrNull()?.text
                    ?: dpia.process!!.key
            )
            dpia.entity != null -> Triple(
                DpiaListItemResponseLinkedResourceType.BUSINESS_ENTITY,
                dpia.entity!!.key,
                dpia.entity!!.names.find { it.locale == "en" }?.text
                    ?: dpia.entity!!.names.firstOrNull()?.text
                    ?: dpia.entity!!.key
            )
