...
open class ProcessFlowMapper {
    ...
    fun toFlowTrackResponse(
        track: ProcessFlowTrack,
    ...
    fun toProcessFlowResponse(
        ...
        val rootNodes =
            (nodesByTrack[null] ?: emptyList())
                .sortedBy { it.position }
                .map { toFlowNodeResponse(it, it.linkedProcessKey != null && it.linkedProcessKey in subProcessKeys) }

        val trackResponses =
            allTracks.sortedBy { it.trackIndex }.map { track ->
    ...
    private fun toApiNodeType(type: FlowNodeType): ApiFlowNodeType =
        when (type) {
    ...
    private fun toApiEventDef(def: FlowEventDefinition): EventDefinition =
        when (def) {
