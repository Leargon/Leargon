...
export async function setup() {
  console.log('[Integration] Creating Docker network...');
  const network = await new Network().start();

  console.log('[Integration] Starting MySQL 8.4 container...');
  ...
  console.log(`[Integration] MySQL ready (host port ${mysql.getPort()}). Starting backend container...`);
  ...
  console.log(`[Integration] Backend ready at ${backendUrl}`);
